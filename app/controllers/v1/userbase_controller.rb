...
class V1::UserbaseController < ApplicationController
  ...
  def update
    ...
    result_data = {user_id:current_user.id,
                   "user-post_code": params["user-post-code"],
                   "user-pref":      params["user-pref"],
    ...
    if @user_lat.blank? || @user_lng.blank?
      ...
      if Base.exists?(user_id: current_user.id)
        ...
        if @base.update( user_id:current_user.id,
                         user_post_code: params["user-post-code"],
                         user_pref:      params["user-pref"],
                         user_city:      params["user-city"],
                         user_area:      params["user-area"],
                         lat:            @user_lat,
                         lng:            @user_lng )
          # update が成功した
        ...
        # 住所の初回登録
        @base = Base.new( user_id:current_user.id,user_post_code: params["user-post-code"],
                          user_pref:      params["user-pref"],
                          user_city:      params["user-city"],
                          user_area:      params["user-area"],
                          lat:            @user_lat,
